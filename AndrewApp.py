...
MSG_TXT = "{\"deviceId\": \"Andrew's Button\",\"Button_Clicked\": \"%s\",\"Timestamp\": \"%s\", \"led\" : \"%s\"}"
...
def iothub_client_sample_run():
    try:
        ...
        #setup an output pin: high input only if button pressed
        GPIO.setup(7,GPIO.OUT)
        #set the LED to "off" by giving the signal 0
        GPIO.output(7,onOrOff)
        ...
        while True:
            ...
            if MESSAGE_SWITCH:
                ...
                # message to indicate a button click
		    ...
		    "On" if not onOrOff else "Off")	
                ...
                # property will tell if LED is ON or OFF
                prop_map = message.properties()
                prop_map.add("LED on", "true" if not onOrOff else "false")
